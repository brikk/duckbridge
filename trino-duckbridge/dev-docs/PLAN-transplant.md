...
- `trino-duckbridge/` — the module verbatim (src, test, resources, dev-docs,
  ...
  application, and repointed the parity-extension root from
  `../duckdb-trino-parity-extension` to the repo-root `duckdb-trino-parity-extension`.
- `build-logic/` — copied the `buildlogic.kotlin.common` + `buildlogic.kotlin.library`
  conventions (kotlin + JDK 25 toolchain, junit/assertj test wiring, custom src
  layout). Dropped `buildlogic.kotlin.brikk` (only wired the TEST-ONLY brikk-sql
  Central Portal snapshots repo, which duckbridge never depends on) and
  `DucklakePostgresService.kt` + the terpal-sql/testcontainers-postgresql/postgres-jdbc
  build-logic deps (terpal-sql plugin support, unused here).
- `gradle/libs.versions.toml` — trimmed to the module + convention refs (trino BOM,
  kotlin BOM, duckdb-jdbc, quack-jdbc, arrow, testcontainers-core, junit, assertj,
  detekt).
- `settings.gradle.kts`, `build.gradle.kts`, `config/detekt/detekt.yml`, gradle
  wrapper, `gradle.properties`, `.gitignore`, `LICENSE`, root `README.md` + `AGENTS.md`.
- `duckdb-trino-parity-extension/` — git submodule pinned to the same commit the
  monorepo pins (`d9fa588232a17c914f3f846e107ef845ce544807`). The pre-built
  `build/release/extension/trino_parity/` output was copied in locally (gitignored)
  so the drift/parity tests run without a 15-min rebuild.

Original design intent (kept for context) below.

---

This module was designed transplant-ready (no `project(":...")` deps, own package
tree `dev.brikk.duckbridge.*`, module-local dev-docs). At the next good stopping
point it moves out of the ducklake monorepo.
