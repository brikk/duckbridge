...
2. ✅ **Numeric / comparison expressions in predicates** — `col + 1 > 5`, `col % 10 = 0`, etc. Same shape, mostly trivial. Pin overflow + decimal semantics.
   ...
   - **Arithmetic operators:** `$add`, `$subtract`, `$multiply` push. `$divide` / `$modulo` do not: Trino integer division truncates where DuckDB `/` is true division, and zero divisors error in Trino but yield Infinity/NULL in DuckDB.
   - **`$coalesce` (variadic), `$nullif` (2-arg), `$identical` (IS NOT DISTINCT FROM)** — translated directly. Shipped round 6f.
   - Function-shape numerics currently pushed: `abs`, `ceil`, `floor`, constant-nonzero-divisor `mod`, `power`, `exp`, `sin`/`cos`/`tan`, `atan`/`atan2`, `sinh`/`cosh`/`tanh`, `degrees`/`radians`, `truncate`, `sign`, `pi`, bitwise and/or/not/xor. `sqrt`, `ln`, `log2`, `log10`, `asin`, `acos` were removed by EV-B2: even a scalar-correct NaN `CASE` rewrite changed WHERE row selection. `cbrt` (precision), shifts, and the non-Trino `bit_length` are also out. Authoritative list: `DuckBridgeEmissionCatalog`.
3. ✅ **String basics** — `SUBSTRING`, `LENGTH`, `LOWER`, `UPPER`, `TRIM`, `CONCAT`, `POSITION`. Watch the Unicode caveat — pin behavior, test against non-ASCII.
   - Shipped: `length/1`, `reverse/1`, `trim/1`, `ltrim/1`, `rtrim/1`, gated `substring/{2,3}`, gated `replace/3`, `strpos/2`, `starts_with/2`, gated `lpad/3`, `rpad/3`, gated `concat_ws/{2..5}`, `translate/3`.
   - **`lower/1`, `upper/1`, `reverse/1` SHIPPED as native C++ in the extension.** ~~DuckDB's built-in `lower`/`upper` do *simple* case folding while Trino's Java implementations do *full* case folding (`lower('İ')` → `'i' + U+0307`, `upper('ß')` → `'SS'`)~~ **CORRECTED 2026-09-02 (EV-E1):** Trino's `lower`/`upper` are *simple* per-code-point mappings too (airlift `SliceUtf8` → `Character.toLowerCase(int)`); `upper('ß')` = `'ß'`, `lower('İ')` = `'i'`. The only real built-in divergence is utf8proc's `upper('ß')` = `'ẞ'`. The extension originally shipped ICU *full* mapping and was wrong on 16/63 corpus inputs; it now uses `u_tolower`/`u_toupper` per code point. DuckDB's `reverse` is grapheme-cluster-aware while Trino's is code-point-only. Collations / `nfc_normalize` do NOT fix this (verified by the since-deleted `ProbeDuckDbCaseFolding`). These were initially shipped as pushed *placeholders* (ASCII-safe, warn-on-emit), then replaced by native ICU-backed C++ functions in `trino_parity` (`src/string_functions.cpp`) that match Trino's simple-mapping / code-point semantics. They are now in `trino_meta()` + `PUSHABLE_FUNCTIONS`, pinned by `TestTrinoFunctionAliases#testRepresentativeAliasSemantics` (Trino-verified values) and the extension's `test/sql/trino_parity.test`.
   ...
   - Regex (Trino Joni vs DuckDB RE2): `regexp_like/2`, `regexp_extract/{2,3}`, `regexp_replace/{2,3}` push only for a constant pattern accepted by `Re2Safety`; extract wraps no-match to NULL and replace/3 additionally rejects `$`/`\` replacement syntax. See EV-A6..A8.
...
- ✅ **Done — native DuckDB extension for Trino-equivalent semantics.** The `trino_parity` extension carries native C++ implementations for functions where DuckDB's built-in diverges from Trino in ways collations / SQL wrapping can't fix: `lower`/`upper` (ICU simple per-code-point mapping), `reverse` (code-point), the trim family, `normalize/1` (NFC), and the hash trio `sha512`/`xxhash64`/`hmac_sha256` (vendored xxHash + WjCryptLib). The signed community binary is bundled and signature-verified at `LOAD`. Any future divergence of this shape gets a native function in the same extension rather than a placeholder.
...
- `lower` / `upper` — both engines do *simple* per-code-point mapping; the one divergence is `upper('ß')`: DuckDB (utf8proc) → `'ẞ'` (U+1E9E), Trino → `'ß'` unchanged. (The earlier claim that Trino does *full* folding — `'SS'`, `'i' + U+0307` — was wrong; see TODO-rectify-from-eval.md EV-E1.) Native impl applies ICU `u_tolower`/`u_toupper` per code point.
...
Pinned by `TestTrinoFunctionAliases#testRepresentativeAliasSemantics` (values verified against Trino 483) and the extension's own sqllogic suite.
...
- Step 2 (numeric / comparison) — **shipped, curated**; see priority item 2 and `README-pushdown-reference.md` for the current allowlist. EV-B1/B2 removed scalar-lookalikes that diverge at domain/error/NaN edges rather than preserving historical breadth.
- Step 3 (string basics) — **shipped, curated**; current allowlist + gates in `DuckBridgeEmissionCatalog`, including:
  - Trim family with full Java whitespace set (round 4 fix).
  - `lower/1`, `upper/1`, `reverse/1` shipped as **native C++ in the extension**, fully Trino-aligned on the audited corpus (ICU simple per-code-point case mapping + code-point reverse). See EV-E1/E3.
  - `chr`, `to_hex`, `to_base64`; `url_encode/decode`, `from_hex/from_base64`, and byte-based DuckDB distance functions were removed by EV-A5/A13.
