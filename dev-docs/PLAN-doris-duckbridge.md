...
| P3 | Session-init capability of quack-jdbc / zone-explicit SQL rendering | Any tz-sensitive pushdown (v1 default: gated off) | **RESOLVED 2026-07-19**: `SET TimeZone` DEAD (quack has no such property; BE Hikari has no `connectionInitSql` hook; smuggled `SET;SELECT` works but POISONS the pooled connection). Option 2 (zone-explicit) VIABLE + server-zone-independent — ENABLED: temporal predicates render by column DuckDB type (naive→naive literal, TIMESTAMPTZ→explicit-UTC `+00`, unknown→drop). Fixes a latent naive-vs-TIMESTAMPTZ zone bug (`REPORT-doris-timezone-probe.md`) |
...
| P6 | Doris session `time_zone` visibility from the plugin | §Timezone options 1–2 | **RESOLVED 2026-07-19**: VISIBLE — `ConnectorSession.getTimeZone()` (set from `ctx.getSessionVariable().getTimeZone()`) is threaded to `planScan`. But not NEEDED for the enabled predicates (all zone-independent). tz-*sensitive* pushdown needing session-zone alignment is a noted future upstream SPI/BE ask — no BE hook to set a per-scan zone without pool poisoning (`REPORT-doris-timezone-probe.md`) |
